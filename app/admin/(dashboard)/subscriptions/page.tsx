...
import { getSubscriptionPlans, createSubscriptionPlan } from '@/services/subscription.service';
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border border-gray-200 focus:border-black focus:ring-1 focus:ring-black p-3 rounded-xl transition ${
                form.isTrial ? 'bg-gray-100 cursor-not-allowed' : ''
              }`}
            />
            <p className="text-xs text-gray-500 mt-1">Subscription cost for this plan.</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, durationDays: Number(e.target.value) }))
              }
            />
            <p className="text-xs text-gray-500 mt-1">Validity period in days.</p>
          </div>

          <div className="flex items-center justify-between p-3 border rounded-xl">
            <div>
              <p className="text-sm font-medium">Trial Plan</p>
              <p className="text-xs text-gray-500">Enable if this plan is a free trial.</p>
            </div>
            ...
            <button
              ...
              className={`w-12 h-6 flex items-center rounded-full p-1 transition ${
                form.isTrial ? 'bg-black' : 'bg-gray-300'
              }`}
            >
              <div
                className={`bg-white w-4 h-4 rounded-full shadow-md transform transition ${
                  form.isTrial ? 'translate-x-6' : ''
                }`}
              />
            </button>
          </div>
